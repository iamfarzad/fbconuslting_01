...
sed -i 's/from "next";/from "next";/' src/app/layout.tsx

# Step 2: Replace react-router-dom with Next.js navigation
echo "Replacing react-router-dom with Next.js navigation..."
find src -type f -name "*.tsx" -exec sed -i 's/import.*from "react-router-dom";//g' {} +
find src -type f -name "*.tsx" -exec sed -i 's/useNavigate/useRouter/g' {} +
find src -type f -name "*.tsx" -exec sed -i 's/navigate(/router.push(/g' {} +
...
# Step 7: Clean up and reinstall dependencies
